...
const EditCameraMapModal = () => {
  const { currentMarkers, setCurrentMarkers } = useContext(MapContext)
  ...
  useEffect(() => {
    if(markers.length > 0) {
      ...
      markers.forEach((item, index) => {
        const markerEl = document.querySelector(`.icon-edit-${index}`)
        markerEl.addEventListener('click', function(e) {
            console.log(item)
            cloneEl[0] = {...cloneEl[0], location: [markers[index].location.lng, markers[index].location.lat]}
  ...
  const handleChangeKeyword = (e) => {
    geocoderService.fetchTextToAddress(
      e.target.value,
      0,
      10,
      (result) => {
